...
```
> 异常观测：当 KV Cache 传输失败时，D 节点可能使用错误/空 KV 继续推理，导致乱码（详见 Issues/GLM-5.2-W8A8输出乱码.md）

### 观测 3：GPU 内存层次映射（对应"GPU内存架构"节）
...
> **A2 推理卡算力参数对照**（术语速查.md 引用）：
> - 560 TOPS INT8（8 位整数下每秒 560 万亿次运算）
> - 280 TFLOPS FP16（16 位半精度浮点下每秒 280 万亿次浮点运算）
> - 64 GB HBM（高带宽显存容量）

### 观测 4：Memory-Bound vs Compute-Bound 实证（对应 GPU 内存架构 第 Ⅱ 点）
