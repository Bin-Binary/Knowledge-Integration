...
*本节目的：*交代面向领域Agent的设计初衷，列出要解决的核心问题，作为后续各节（架构/状态/子图/编排/可靠性）的共同决策依据。
*解决的问题：*读者在进入细节前，明确"为什么这样设计"——意图多样性、子图依赖复杂、执行可靠性三个源头制约着后续所有设计选择。

本设计面向领域的Agent，旨在解决以下核心问题：

- 意图多样性：用户需求可能对应不同类型的事件，需要父图根据意图识别结果灵活编排不同的子图流程
- 子图依赖复杂：子图之间可能存在数据依赖（前序输出作为后续输入）和控制依赖（仅当特定条件满足时才执行某些子图），父图必须清晰定义并管理这些依赖
- 执行可靠性：每个子图处理目标必须成功，否则流程应具备重试、回滚、降级或人工介入机制，避免静默失败

*设计原则：*

1. *框架语义驱动：*
LLM先按LU唤起frame schema，以FE填充产出结构化意图，而非自由文本任务单；意图结构由frame schema约束。

2. *描述与执行分离：*
frame是知识系统的边界，描述侧产出意图（frame快照）、执行侧回流结果（result）。
父图与子图皆横跨该边界：意图识别/规划属描述侧，纯函数落地/依赖输配/重试回滚属执行侧。描述→执行是可重入循环：子图内plan→execute→validate→replan沿边界闭环，父图是对多个子图循环的编排轴。frame即这条边界，也是知识与能力的分界。

3. *两级编排（组织轴）：*
父图=supervisor（按意图识别结果选子图、管理依赖、人工介入闸、降级兜底）负责编排；子图=plan-and-execute（计划-执行-校验循环，重试/回滚靠快照链）负责执行任务。
父图与子图是组织粒度的差异，独立于原则2的描述/执行职责轴：父图有意图识别（描述侧）也有编排兜底（执行侧），子图有规划（描述侧）也有执行单元（执行侧），
两者都横跨描述/执行边界，不要将父图对应"描述层"、子图对应"执行层"。
